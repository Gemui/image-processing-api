...
class HandleImageData {
    ...
    public findOrCreateImage(): sharp.Sharp {
        this.file = sharp(path.resolve(this.imagePath,`full`, `${this.requestQuery.filename}`));
    ...
    public async saveFileToDisk(): Promise<string> {
        ...
        const newImagePath = path.resolve(this.imagePath, 'thumb',`${this.imageName}`);
   ...
   private getAsNumber(word: string | string[] | qs.ParsedQs | qs.ParsedQs[]): number {
